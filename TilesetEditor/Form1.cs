...
namespace TilesetEditor
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            btnUndo.Click += (s, e) => { undo.Undo(); tileSet.RebuildTiles(); panelTileset.Invalidate(); UpdateDirtyFromUndoRedo(); };
            btnRedo.Click += (s, e) => { undo.Redo(); tileSet.RebuildTiles(); panelTileset.Invalidate(); UpdateDirtyFromUndoRedo(); };
        ...
        private void UpdateDirtyFromUndoRedo()
        {
            // simple heuristic: if undo stack empty => not dirty
            // (UndoManager currently doesn't expose counts; if needed extend it)
            // For now, keep isDirty as-is; better approach: expose counts in UndoManager.
        }
        #endregion
        ...
        {
            if (tileSet.Image == null) { MessageBox.Show("Load a tileset first."); return; }
            ApplyTileSizeAndRefresh();
        }
        ...
        {
            ApplyTileSize();
            // If using custom grid, tileSet.Columns/Rows come from GridCols/GridRows
            // We DO NOT change tileSet.Image size here (image remains original)
            tileSet.RebuildTiles();
            UpdateScrollSize();
            EndDrag();
            selectedIndex = -1;
            panelTileset.Invalidate();
            UpdateStatus($"Tiles: {tileSet.Tiles.Count}  (Cols:{tileSet.Columns} Rows:{tileSet.Rows})");
        }
        ...
        private void MiSave_Click(object sender, EventArgs e)
        {
            ...
            using var sfd = new SaveFileDialog { Filter = "PNG Image|*.png|JPEG Image|*.jpg;*.jpeg|Bitmap|*.bmp", FileName = "tileset.png" };
            if (sfd.ShowDialog() != DialogResult.OK) return;

            // Save current tileSet.Image to file
